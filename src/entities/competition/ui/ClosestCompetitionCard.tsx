...
export const ClosestCompetitionCard: React.FC<ClosestCompetitionCardProps> = (
  props
) => {
  const [timeLeft, setTimeLeft] = useState(calculateTimeLeft(props.competition ? props.competition.date : ""));

  useEffect(() => {
    const timer = setInterval(() => {
      setTimeLeft(calculateTimeLeft(props.competition ? props.competition.date : ""));
    }, 1000);
    ...
    return (
      <div className="flex flex-col w-full h-[330px] rounded-2xl container-shadow px-8 py-6 gap-2">
        ...
        <div className="flex flex-col gap-1">
          ...
          {Array(3).fill(0).map((_event, index) => (
